...
class TimSort : public Sort<T, DisplayType> {
   private:
    ...
    // 插入排序：对小块进行插入排序
    void insertionSort(size_t left, size_t right, int speed) {
    ...
    // 合并两个已排序的区间
    void merge(size_t left, size_t mid, size_t right, int speed) {
    ...
    // 创建 runs 并进行插入排序
    void createRuns(int speed) {
        size_t n = this->arr.size();
        for (size_t start = 0; start < n; start += MIN_RUN) {
            size_t end = std::min(start + MIN_RUN - 1, n - 1);
            insertionSort(start, end, speed);
        }
    }

    // TimSort 主排序函数
    void timSort(int speed) {
        this->functionCalls++;  // 统计 TimSort 调用次数
        createRuns(speed);      // Step 1: 创建 runs 并插入排序
        timSortMerge(speed);    // Step 2: 合并 runs
    }
